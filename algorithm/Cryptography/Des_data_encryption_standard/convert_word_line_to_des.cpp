...
string toBinary(std::string const &str)
{
    string final_binary= "";
    std::string binary = "";
    ...
    for(int i=0;i<(64-binary.size());i++){
        final_binary+='0';
    }
    final_binary+=binary;
    return final_binary;
...
string convert_to_hes (string str2){
    //A 64 bit text
    string plain_text = str2;
    string temp_plain_text = "";
    temp_plain_text = toBinary(plain_text);
    string key = "1010101010111011000010010001100000100111001101101100110011011101";
    //A block of plain text 64 bits
    pt = temp_plain_text;
    string apt = pt;
    //calling the function to generate 16 keys
    generate_keys(key);
    cout<<"Plain Text Word Binary : "<<pt<<endl;
    //apply the algo
    string ct = DES();
...
int main()
{
    ...
    while(iss){
        ...
        string temp_str = convert_to_hes(str2);

        for (int i = 0; i < temp_str.size();i++){
            if((i+1)%8 ==0){
               temp_str2.push_back(temp_str[i]);
               temp_str2.push_back(' ');
            } else {
                temp_str2.push_back(temp_str[i]);
            }
        ...
        V.push_back(temp_str2);
    }
    for(int i=0;i<V.size()-1;i++){
        cout<<"word "<<i<<" "<<V[i]<<endl;
    }
